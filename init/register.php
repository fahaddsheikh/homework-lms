...
register_activation_hook( __FILE__, 'homework_lms_activate' );
/**
 * Add a flag that will allow to flush the rewrite rules when needed.
 */
function homework_lms_activate() {
    if ( ! get_option( 'homework_lms_flush_rewrite_rules_flag' ) ) {
        add_option( 'homework_lms_flush_rewrite_rules_flag', true );
    }
}
 ...
 */
function homework_lms_flush_rewrite_rules_maybe() {
    if ( get_option( 'homework_lms_flush_rewrite_rules_flag' ) ) {
        flush_rewrite_rules();
        delete_option( 'homework_lms_flush_rewrite_rules_flag' );
    }
}
add_action( 'init', 'homework_lms_flush_rewrite_rules_maybe', 20 );
