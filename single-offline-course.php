...
<section class="container page-content">
	...
	if($michigan_webnus_options['michigan_webnus_enable_breadcrumbs']){
		...
		$w_courses = get_post_type_object( 'course_offline' );
	...
	<div class="course-main">
		<?php if( have_posts() ): while( have_posts() ): the_post();
			$post_id = get_the_ID();
			$batches = get_post_meta( $post_id, 'batch_for_course', true );
			if (isset($batches) && !empty($batches)) {
				$batchesarray = explode(",",$batches);
				foreach ($batchesarray as $batch) {
		    		$price[] = get_post_meta( $batch, 'price', true );
		    		$price[] = get_post_meta( $batch, 'discounted_price', true );
		    		$latestbatches[$batch] = strtotime(get_post_meta( $batch, 'start_date', true ));
		    	}
    	    	asort($latestbatches);
		...
		<section class="col-md-9 course-content cntt-w">
				...
				<div class="container">
					...
					<?php if (!empty($batchesarray)) : ?>
						<div class="course-features clearfix col-md-12 online-t">
						    ...
						    <?php
				    		foreach ($latestbatches as $batch => $val) :
				    			$batchnoset = get_post_meta( $batch, 'batch_number', true );
				        		$startdate = strtotime(get_post_meta( $batch, 'start_date', true )); 
				        		$enddate = strtotime(get_post_meta( $batch, 'end_date', true ));
				        		$duration = get_post_meta( $batch, 'duration', true );
				        		$days = get_post_meta( $batch, 'days', true );
				        		$time = get_post_meta( $batch, 'time', true );
				        		$price = get_post_meta( $batch, 'price', true );
				        		$discounted_price = get_post_meta( $batch, 'discounted_price', true );
				        		$venue = get_post_meta( $batch, 'venue', true );
				        		$number_registered_students = get_post_meta( $batch, 'number_registered_students', true );
				        		$number_total_students = get_post_meta( $batch, 'number_total_students', true ); 
				        		$discount_detail = get_post_meta( $batch, 'discount_detail', true );
				        		...
				        		if ($enddate > date("Y-m-d")) : ?>
				        			<div id="batch-no-<?php echo $batchnoset ?>" class="single-batch">
								        ...
								        <?php if(isset($discounted_price) && !empty($discounted_price)): echo "<span id='discounted-price'>Rs. ". $discounted_price ." </span><span id='original-price'>Rs.".  $price . "</span>"; else:  echo "Rs." . $price; endif;?>
								        	<?php 
								        		if(isset($discount_detail) && !empty($discount_detail)): 
								        			echo "<span class='discount-detail'>" . $discount_detail . "</span>"; 
								        		endif;
								        	?>
								        </div>
			...
			$args = array('post__not_in' => $post_ids,'showposts' => 3,'orderby'=>'date','order'=>'desc','post_type'=>'course_offline','author' => $author_id,);
